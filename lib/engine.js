...
function renderPanel(key) {
  ...
  if (key === "about" && data.cards) {
    // Konoha forehead-protector, drawn as inline SVG — a self-contained stand-in
    // for a Naruto photo (replaceable via card.photo → /public image).
    const headband = `
      <span class="note-band" aria-hidden="true">
        <svg viewBox="0 0 160 120">
          <defs>
            <linearGradient id="plateGrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0" stop-color="#f1f3f6"/>
              <stop offset="0.5" stop-color="#cbd0d8"/>
              <stop offset="1" stop-color="#a6adb8"/>
            </linearGradient>
          </defs>
          <rect x="0" y="48" width="160" height="24" fill="#212b39"/>
          <rect x="0" y="46" width="160" height="3" fill="#0e1620"/>
          <rect x="0" y="71" width="160" height="3" fill="#0e1620"/>
          <line x1="0" y1="52.5" x2="160" y2="52.5" stroke="#364353" stroke-width="1" stroke-dasharray="5 4"/>
          <line x1="0" y1="67.5" x2="160" y2="67.5" stroke="#364353" stroke-width="1" stroke-dasharray="5 4"/>
          <rect x="38" y="34" width="84" height="52" rx="8" fill="url(#plateGrad)" stroke="#868d97" stroke-width="1.5"/>
          <rect x="43" y="37" width="74" height="9" rx="4.5" fill="#ffffff" opacity="0.34"/>
          <circle cx="47" cy="41" r="2.1" fill="#868d97"/>
          <circle cx="113" cy="41" r="2.1" fill="#868d97"/>
          <circle cx="47" cy="79" r="2.1" fill="#868d97"/>
          <circle cx="113" cy="79" r="2.1" fill="#868d97"/>
          <g transform="translate(80,60)">
            <g class="band-swirl" fill="none" stroke="#2b333d" stroke-width="4.4" stroke-linecap="round" stroke-linejoin="round">
              <path d="M0 -15 C 15 -15 18 6 2 9 C -10 11 -13 -3 -1 -6 C 6 -7.5 8 2 1 3"/>
              <line x1="-2.5" y1="-15" x2="-13" y2="6"/>
            </g>
          </g>
        </svg>
      </span>`;
    ...
    html += `<div class="note-board">${data.cards.map((card, index) => {
      ...
      if (card.type === "photo") {
        const inner = card.photo
          ...
          : headband;
        return `
          <figure class="note note--photo stagger-item" style="animation-delay: ${140 + index * 70}ms">
            ${pin}
            <div class="note-photo">${inner}</div>
            <figcaption>${t(card.caption)}</figcaption>
          </figure>`;
      }

      return `
        <div class="note note--${card.type} stagger-item" style="animation-delay: ${140 + index * 70}ms">
          ${pin}
