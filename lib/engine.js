...
const colors = ["#8FAEFF", "#D1D5DB", "#BFC7D5", "#8F1D2C", "#6FE0E8", "#B98FFF"];
...
function renderPanel(key) {
  ...
  // ── About: RPG player card + human cards ─────────────────────────────────
  if (key === "about" && data.player) {
    const p = data.player;
    html += `
      <div class="player-card stagger-item" style="animation-delay: 90ms">
        <div class="player-card__head">
          <span class="player-card__badge">PLAYER 1</span>
          <div class="player-card__id">
            <strong>${p.name}</strong>
            <span class="player-card__class">${t(p.klass)}</span>
          </div>
          <span class="player-card__hearts" aria-hidden="true">❤️❤️❤️</span>
        </div>
        <div class="player-card__stats">
          ${p.stats.map((s, i) => `
            <div class="pstat" style="--pv:${s.value}%; --pd:${350 + i * 140}ms">
              <span class="pstat__label">${t(s.label)}</span>
              <span class="pstat__bar"><span class="pstat__fill"></span></span>
              <span class="pstat__num">${s.value}</span>
            </div>`).join("")}
        </div>
        <div class="player-card__achv">
          ${p.achievements.map(a => `<span class="achv"><em>${a.icon}</em>${t(a.text)}</span>`).join("")}
        </div>
      </div>`;
  }
  ...
  if (key === "about" && data.cards) {
    const shuriken = `
      <span class="about-deco" aria-hidden="true">
        <svg viewBox="0 0 100 100" fill="currentColor">
          <path d="M50 2 L61 39 L98 50 L61 61 L50 98 L39 61 L2 50 L39 39 Z"/>
          <circle cx="50" cy="50" r="7" fill="#0b0e14"/>
        </svg>
      </span>`;
    html += `<div class="about-grid">${data.cards.map((card, index) => {
      const link = card.link
        ? `<a class="about-link" href="${card.link.href}" target="_blank" rel="noopener">${t(card.link.label)} <i class="fa-solid fa-arrow-up-right"></i></a>`
        : "";
      const deco = card.type === "anime" ? shuriken : "";
      return `
        <div class="about-card about-card--${card.type} stagger-item" style="animation-delay: ${150 + index * 70}ms">
          ${deco}
          <span class="about-ico"><i class="${card.icon}"></i></span>
